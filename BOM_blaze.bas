Attribute VB_Name = "BOM_v5_blaze"
'Version 0.5 blaze : supports both shoes and slippers
...
'SCS
    If C_LOOK("SCS", "B") > 0 Then
        SLITTED_UPPER "4-scs-" & article, True
    End If
'SCS1
    If C_LOOK("SCS1", "B") > 0 Then
        SLITTED_UPPER "4-scs1-" & article, False
    End If
'SCS2
    If C_LOOK("SCS2", "B") > 0 Then
        SLITTED_UPPER "4-scs2-" & article, False
    End If
...
Sub SLITTED_UPPER(ite As String, oh As Boolean)
    itemCount = 0
    Select Case ite
        Case "4-scs-" & article
            cellX = CELL_X("SCS")
