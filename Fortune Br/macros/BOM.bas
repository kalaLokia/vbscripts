...
Sub ArticleDetails(Optional temp As Workbook)
    ...
    ElseIf Article.utils = "P48" Then
        Article.IsExport = True
        Article.BrandCode = Article.BrandCode & "-P48"
    ElseIf Article.utils = "P60" Then
        Article.IsExport = True
        Article.BrandCode = Article.BrandCode & "-P60"
    Else:
...
Sub MASTER_CARTON(ite As String)
    ...
    itemCount = 0
    cellX = CELL_X("MC")
    father = ite & scCount(6)
    childitem = "3-fb-"
    article_id = Article.id
    If Article.Category = "X" And Article.IsExport = False Then
        father = father & "-P24"
    ElseIf Article.utils = "E-POLY24" Then
        father = father & "-P24"
        childitem = "4-mpu-"
        article_id = Left(Article.name, Len(Article.name) - 1) & "-" & Article.Color & "-" & Article.Category
    ElseIf Article.utils = "P48" Then
        father = father & "-P48"
        childitem = "4-mpu-"
    ElseIf Article.utils = "P60" Then
    ...
    For i = 0 To siz
        LINE_CELLS father, itemCount, childitem & article_id & WorksheetFunction.Text(Article.size(0) + i, "00"), scCount(i), "4"
    Next i
    For j = 0 To cellX(1) - 1
        If IsEmpty(Worksheets("BOM").Range("D" & cellX(0) + j).value) = False Then
            LINE_CELLS father, itemCount, Worksheets("BOM").Range("D" & cellX(0) + j), Worksheets("BOM").cellS(cellX(0) + j, 6), 4
        End If
    Next j
    LINE_CELLS father, itemCount, "FGMC-OH", scCount(i), 290
...
Sub FINISHED_UPPER(ite As String)
    ...
    For i = 0 To siz
        ...
        If C_LOOK("CCS", "B") > 0 Then
            If Worksheets("BOM").Range("A" & C_LOOK("CCS", "B")) = "#" Then
                LINE_CELLS ite & WorksheetFunction.Text(Article.size(0) + i, "00"), itemCount, "4-ccs-" & Article.id, 1, 4
                Else:
...
Sub FOLDED_UPPER(ite As String)
    ...
    For k = i To j
        If IsEmpty(Worksheets("BOM").Range("D" & k)) = False Then
            If (InStr(1, Worksheets("BOM").cellS(k, 3), "REXINE", vbTextCompare) = 1) Or (Worksheets("BOM").cellS(k, 4) Like "5-NL*") Then
                If h > 0 Then
                    slit = "4-scf" & h & "-"
                End If
        ...
        'Slitting item under the folding
        h = 0
        For k = i To j
            itemCount = 0
            If InStr(1, Worksheets("BOM").cellS(k, 3), "REXINE", vbTextCompare) = 1 Or (Worksheets("BOM").cellS(k, 4) Like "5-NL*") Then
                ...
                End If
                h = h + 1
                LINE_CELLS slit & Article.id, itemCount, Worksheets("BOM").Range("D" & k), Worksheets("BOM").cellS(k, 7 + siz), 4
                If slit = "4-scf-" Then
...
Sub BOM_CELLS(valueC, valueD, valueE, rowNo As Integer, Optional multiplier As Single = 1, Optional valueX As Single = 0)
    ...
    For colNo = 6 To 10
        If Worksheets("BOM").cellS(rowNo, colNo).value > 0 Then
            If multiplier > 0 Then
                swVal = Worksheets("BOM").cellS(rowNo, colNo).value * multiplier
                Worksheets("BOM").cellS(rwc, colNo).value = Round(swVal, 4)
            Else
                Worksheets("BOM").cellS(rwc, colNo).value = Round(valueX, 4)
            End If
        End If
    Next colNo
...
Sub SOLE_ITEMS()
    ...
    PUX = GET_SOLE_COLOR(Worksheets("BOM").Range("C" & C_LOOK("SOLE", "B")).value)
    cellX = CELL_X("MPU")
    rwc = cellX(0)
    'SHOES
    If C_LOOK("JBLD", "C") > 0 Then
        BOM_CELLS "OUTTER SOLE", "4-PUX-0040", Worksheets("BOM").Range("E" & C_LOOK("SOLE", "B")).value, C_LOOK("SOLE", "B")
        BOM_CELLS "MID SOLE[i]", "5-PO01-0043", "PU-JBLD ISO MDI 2509/IN", C_LOOK("SOLE", "B") + 1, 72 / 175
        BOM_CELLS "MID SOLE[p]", "5-PO01-0044", "PU-JBLD POLYOL 721", C_LOOK("SOLE", "B") + 1, 100 / 175 * 200 / 225
        BOM_CELLS "ADDITIVE[p]", "6-CHM-0146", "JBLD ADDITIVE P721/3/200", C_LOOK("SOLE", "B") + 1, 100 / 175 * 25 / 225
        BOM_CELLS "MID SOLE[c]", "6-CHM-0019", "PIGMENT KC 1871 WHITE", C_LOOK("SOLE", "B") + 1, 3 / 175
        BOM_CELLS "IMC -WH", "6-CHM-0156", "Water Base IMC White KB 4505", C_LOOK("SOLE", "B"), 0, 0.005
        BOM_CELLS "WB 07A", "6-CHM-0126", "RELEASE AGENT W.B 711/07A", C_LOOK("SOLE", "B"), 0, 0.001
        BOM_CELLS "1602", "6-CHM-0010", "RELEASE AGENT KECK? 1602/18", C_LOOK("SOLE", "B"), 0, 0.001
        
        'DOUBLE COLOR
    ElseIf C_LOOK("2 COLOR", "C") > 0 Then
        BOM_CELLS "OUTTER SOLE", "4-PUX-0004", Worksheets("BOM").Range("E" & C_LOOK("SOLE", "B")).value, C_LOOK("SOLE", "B")
        BOM_CELLS "MID SOLE[i]", "5-PO01-0004", "ISO 163", C_LOOK("SOLE", "B") + 1, 51 / 154
        BOM_CELLS "MID SOLE[p]", "5-PO01-0042", "POLY VB1", C_LOOK("SOLE", "B") + 1, 100 / 154
        BOM_CELLS "MID SOLE[c]", "6-CHM-0019", "PIGMENT KC 1871 WHITE", C_LOOK("SOLE", "B") + 1, 3 / 154
        BOM_CELLS "IMC -WH", "6-CHM-0156", "Water Base IMC White KB 4505", C_LOOK("SOLE", "B"), 0, 0.004
        BOM_CELLS "WB 07A", "6-CHM-0126", "RELEASE AGENT W.B 711/07A", C_LOOK("SOLE", "B"), 0, 0.001
        BOM_CELLS "1602", "6-CHM-0010", "RELEASE AGENT KECK? 1602/18", C_LOOK("SOLE", "B"), 0, 0.001

    Else:
        BOM_CELLS "OUTTER SOLE", PUX, Worksheets("BOM").Range("E" & C_LOOK("SOLE", "B")).value, C_LOOK("SOLE", "B")
        'SOFT
        If C_LOOK("SOFT", "C") > 0 Then
            'HUNTSMAN SOFT SOLE
            BOM_CELLS "MID SOLE[i]", "5-PO01-0002", "ISO  HUNTSMAN [SUPRASEC 2442]", C_LOOK("SOLE", "B") + 1, 54 / 154
            BOM_CELLS "MID SOLE[p]", "5-PO01-0006", "POLY HUNTSMAN [DALTOPED AF 35100 (MIDSOLE - DOUBLE DENSITY)]", C_LOOK("SOLE", "B") + 1, 100 / 154
            'Double density chem
            BOM_CELLS "1602", "6-CHM-0010", "RELEASE AGENT KECK  1602/18", C_LOOK("SOLE", "B"), 0, 0.001
        ...
        End If
        BOM_CELLS "ELFI", "6-ADH-0029", "ELFI", C_LOOK("SOLE", "B"), 0, 0.0003
        BOM_CELLS "WB 07A", "6-CHM-0126", "RELEASE AGENT W.B 711/07A", C_LOOK("SOLE", "B"), 0, 0.001
    End If
...
' SOLE COLOR SETTING
Function GET_SOLE_COLOR(Col As String) As String
...
Function MC_ITEMS(SIZEE As String) As Integer()
    ...
    Case "G_6X10-E-P60"
        ...
        '###### Special Andaman Case inside Case ######
     Case "G_6X10-P48"
        sc_count(0) = 12
        sc_count(1) = 12
        sc_count(2) = 12
